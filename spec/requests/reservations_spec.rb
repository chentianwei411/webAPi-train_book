...
RSpec.describe "Reservations", type: :request do
  before do
    ...
    @user = User.create!(:email => "test@example.com", :password => "123213")
    @reservation = Reservation.create!(:train_id => @train1.id, :seat_number => "1A", :customer_name => "foo", :customer_phone => "123123")
  end

  it "GET /api/v1/reservations/{booking_code}" do
  ...
  it "DELETE /api/v1/reservations/{booking_code}" do
    delete "/api/v1/reservations/#{@reservation.booking_code}"
    expect(response).to have_http_status(200)
    expect(response.body).to eq( { :message => "已经取消订位" }.to_json )
    expect( Reservation.count ).to eq(0)
  end

  it "PATCH /api/v1/reservations/{booking_code}" do
  ...
  describe "POST /api/v1/reservations" do
    it "Success without auth_thoken" do
      ...
      expect(created_reservation.customer_phone).to eq("555555")
      expect(created_reservation.user_id).to eq(nil)
    end

    it "Success with auth_thoken" do
      post "/api/v1/reservations", :params => { :auth_token => @user.authenication_token,
    ...
    it "failed, because same seat number" do
  ...
  describe "GET /api/v1/reservations" do
    ...
    it "is success" do
      ...
      get "/api/v1/reservations", :params => { :auth_token => @user.authenication_token }
